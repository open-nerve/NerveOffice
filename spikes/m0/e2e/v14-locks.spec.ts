...
// - 编辑中的标签页持有文档的排他锁；另一个标签页拿不到锁，不写发件箱，排队等锁；
...
import { edit, openWithOutbox, outboxInfo, storeFixture, waitSavedLocal } from './p6-helpers';
...
test('V14 标签页协调：锁被抢走后停止写入', async ({ context, request }, testInfo) => {
    test.setTimeout(120_000);
    const id = `p6-steal-${testInfo.project.name}`;
    await storeFixture(request, SERVERS.full, 'sheet', 'sheet-core', id);
    const a = await context.newPage();
    await openWithOutbox(a, SERVERS.full, { kind: 'sheet', doc: id });
    const b = await context.newPage();
    await b.goto(`${SERVERS.full}/index.html`);
    // B 用 steal 抢锁（生产上对应"在这里继续编辑"）
    await b.evaluate((name) => {
        void navigator.locks.request(name, { steal: true }, () => new Promise(() => undefined));
    }, `nerve-doc:${id}`);
    const lost = await waitState(a, ['lost']);
    const before = await writes(a);
    await edit(a, 'sheet', 'P6S1');
    await a.waitForTimeout(2000);
    const afterWrites = await writes(a);
    const captureError = await a.evaluate(() => window.__m0!.outbox!.capture().then(() => null, (e: Error) => e.message));
    await writeResult(`v14/locks/${testInfo.project.name}-steal.json`, {
        check: 'V14-locks-steal', browser: browserInfo(a, testInfo), timestamp: new Date().toISOString(), lost, before, afterWrites, captureError,
    });
    expect(lost.state, 'A 收到锁被抢走').toBe('lost');
    expect(afterWrites, '之后的修改不写发件箱').toBe(before);
    expect(captureError, '手动写入也被拒绝').not.toBeNull();
});
