...
//   2. 修改输入 → 立即捕获（不等待）；
//   3. 等 onCalculationResultApplied 后捕获（等待）；
//   4. 再修改一次输入 → 用 50 ms 的超时强制走超时 → 捕获（标记"公式待更新"）→ 等计算结束后补捕获；
//   5. 把"等待"的快照写回存储：默认模式重开，显示值应等于快照值；forced 模式重开，SDK 重算的结果应等于快照值。
...
import { brief, detectorMark, detectorState, ensureGenerated } from './p3-helpers';
...
for (const worker of [false, true]) {
    for (const s of SCENARIOS) {
        ...
        test(`V07 公式一致性：${name}`, async ({ page, request }, testInfo) => {
            ...
            await request.put(`${SERVERS.off}/api/docs/${id}`, { data: JSON.parse(waited.text) });
            ...
            const result = {
                ...
                waitMs: { noWait: noWait.waitMs, waited: waited.waitMs, timeoutAttempt: timedOut.waitMs, recapture: recapture.waitMs },
            ...
            // 平台需要的保证：等待后的快照、超时后的补捕获、重开与强制重算，都与公式定义一致
            expect.soft(checks.waited.mismatches, '等待后捕获').toEqual([]);
            expect.soft(checks.recapture.mismatches, '超时后补捕获').toEqual([]);
            expect.soft(checks.reopened.mismatches, '默认模式重开').toEqual([]);
            expect.soft(checks.forced.mismatches, '强制重算').toEqual([]);
            expect.soft(checks.waited.checked, '核对了单元格').toBeGreaterThan(0);
        });
    }
}
