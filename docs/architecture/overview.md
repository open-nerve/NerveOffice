...
- **api**：
  ...
  - 只有 database 模块、各模块的仓储与表定义能引用 `drizzle-orm`、`pg`，控制器不引用仓储；
  - 只有 config 模块读取 `process.env`；
  - 控制器的输入必须带 schema，不用 `@Req`、`@Res`；
  - SQL 只用参数，不拼接；
  - 不用 Nest 的静态 `Logger`。
...
| 表 | 模块 | 说明 |
|---|---|---|
| `audit_events` | audit | 审计事件：动作、操作者、对象、来源（请求标识、客户端地址）、补充信息；CHECK 约束兜底；触发器拒绝更新、删除与清空 |
...
| 2026-09-26 | M1-P2 | 后端骨架与横切能力；后端的模块边界；错误码；数据库与迁移、`migrations` 检查；审计；覆盖率改为单元与集成测试合计 |
